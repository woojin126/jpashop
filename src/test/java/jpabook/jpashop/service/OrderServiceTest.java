...
public class OrderServiceTest {
    ...
    public void 주문취소() throws Exception{
        ...
        Assertions.assertEquals(10,item.getStockQuantity(),"주문이 취소된 상품은 그만큼 재고가 원복 되야한다");

    }

    @Test(expected = NotEnoughStockException.class)//주문 재고수량 초과하면 이게터지도록 설정해놨다.
    ...
    private Book createBook(String name, int price, int stockQuantity) {//컨트롤+알트 +P  파라미터로 뽑기가능
        Book book = new Book();
        book.setName(name);
        book.setPrice(price);
